...
//   if (static_cast<unsigned int>(field_num.asInt()) != field_value.size()) {
...
void IndexMeta::to_json(Json::Value &json_value) const
{
  ...
  json_value[FIELD_FIELD_NAME] = ss.str().c_str();
}
...
RC IndexMeta::from_json(const TableMeta &table, const Json::Value &json_value, IndexMeta &index)
{
  ...
  const char*  field_field_name=field_value.asCString();
  std::vector<std::string> result;
  std::stringstream ss(field_field_name);
  std::string token;
  ...
  for (size_t i = 0; i < result.size(); i++) {
    const FieldMeta *field = table.field(result[i].c_str());
    if (nullptr == field) {
      LOG_ERROR("Deserialize index [%s]: no such field: %s", name_value.asCString(), field_value.asCString());
      return RC::SCHEMA_FIELD_MISSING;
    }
